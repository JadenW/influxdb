...
func TestSwitchFillImplRule(t *testing.T) {
	...
	withFlagger, _ := feature.Annotate(context.Background(), flagger)
	fillProcedureSpec := &universe.FillProcedureSpec{
		DefaultCost: plan.DefaultCost{},
		Column:      "_value",
		Value:       values.NewFloat(0),
		UsePrevious: false,
	}
	targetSpec := fillProcedureSpec.Copy().(*universe.FillProcedureSpec)
	targetSpec.Fallback()
	testcases := []plantest.RuleTestCase{
		{
			...
			Before: &plantest.PlanSpec{
				Nodes: []plan.Node{
					plan.CreatePhysicalNode("fill", fillProcedureSpec),
				},
			},
			NoChange: true,
		},
